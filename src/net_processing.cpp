...
struct CNodeState {
    ...
    struct InventoryDownloadState {
        ...
        std::map<CInv, std::chrono::microseconds> m_inv_in_flight;
...
limitedmap<CInv, std::chrono::microseconds> g_already_data_asked_for GUARDED_BY(cs_main)(MAX_INV_SZ);
...
static void PushNodeVersion(CNode* pnode, CConnman* connman, int64_t nTime)
{
    ...
    connman->PushMessage(pnode, CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::VERSION, PROTOCOL_VERSION, (uint64_t)nLocalNodeServices, nTime, addrYou, addrMe, nonce, strSubVersion, nNodeStartingHeight, ::g_relay_txes, mnauthChallenge));
...
std::chrono::microseconds GetDataRequestTime(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...
void UpdateDataRequestTime(const CInv& inv, std::chrono::microseconds request_time) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    auto it = g_already_data_asked_for.find(inv);
    if (it == g_already_data_asked_for.end()) {
...
std::chrono::microseconds CalculateDataGetDataTime(const CInv& inv, std::chrono::microseconds current_time, bool use_inbound_delay) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    std::chrono::microseconds process_time;
    const auto last_request_time = GetDataRequestTime(inv);
    // First time requesting this tx
    if (last_request_time == (std::chrono::microseconds)0) {
        process_time = current_time;
    } else {
        // Randomize the delay to avoid biasing some peers over others (such as due to
        // fixed ordering of peer processing in ThreadMessageHandler)
...
void RequestData(const NodeId id, const CInv& inv)
{
    ...
    // Calculate the time to try requesting this transaction. Use
    // fPreferredDownload as a proxy for outbound peers.
    std::chrono::microseconds process_time = CalculateDataGetDataTime(inv, current_time, !state->fPreferredDownload);
...
static void RelayAddress(const CAddress& addr, bool fReachable, const CConnman& connman)
{
    ...
    // at a time so the m_addr_known of the chosen nodes prevent repeats
...
bool ProcessMessage(CNode* pfrom, const std::string& msg_type, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CTxMemPool& mempool, CConnman* connman, BanMan* banman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::NOTFOUND) {
        ...
        if (found) {
            ...
            llmq::quorumInstantSendManager->ProcessMessage(pfrom, msg_type, vRecv, connman);
            return true;
        }
    }
...
bool PeerLogicValidation::SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (!::ChainstateActive().IsInitialBlockDownload() && pto->m_next_local_addr_send < current_time) {
            AdvertiseLocal(pto);
